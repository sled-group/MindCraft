...
class Model(nn.Module):
    def __init__(self, seq_model_type=0):
        ...
        if seq_model_type==0:
            ...
            self.dialogue_listener_lin1 = nn.Linear(drnn_in,dlist_hidden)
            self.dialogue_listener_attn = nn.MultiheadAttention(dlist_hidden, 8)
            self.dialogue_listener_wrap = lambda x: self.dialogue_listener_attn(x,x,x)
            self.dialogue_listener = lambda x: self.dialogue_listener_wrap(
                self.dialogue_listener_lin1(x).reshape(-1,1,dlist_hidden)
            )[0]
        else:
    ...
    def forward(self,game):
        ...
        q, l = zip(*q)
        # f = self.conv(torch.tensor(f).permute(0,3,1,2).float().to(DEVICE)).reshape(-1,512)
        # print(f.shape,torch.tensor(d).shape,torch.tensor(q).shape)
        

        # print(self.conv(torch.tensor(f).permute(0,3,1,2).float().to(DEVICE)).shape)
        ...
        # u = u.reshape(u.shape[0],1,u.shape[1]).float().to(DEVICE)
        u = u.float().to(DEVICE)
        ...
        # fun_lst = [self.q01,self.q02,self.q03]
